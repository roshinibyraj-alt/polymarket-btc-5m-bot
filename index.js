...
<title>30 Seconds Waiter — CheapHunter</title>
<style>
...
<div class="section-head"><span>CheapHunter — wait 30s → buy underdog ≤ 0.20 · TP @ 0.50 · no SL · no martingale</span></div>
...
const eh=$('entryHint');const waitSec=(d.config&&d.config.waitSeconds)||7;if(d.windowPaused){eh.textContent='⛔ '+ESC(d.pauseReason||'PAUSED')}else if(d.waitingForWindow){eh.textContent='WAITING FOR NEXT WINDOW'}else if(d.windowElapsed!=null&&d.windowElapsed<waitSec){eh.textContent='WAIT '+(waitSec-d.windowElapsed)+'s → buy underdog ≤ 0.20'}else if(d.openEntry){eh.textContent='HOLDING '+(d.openEntry==='UP'?'▲ UP':'▼ DOWN')+' · TP @ 0.50 · HOLD TO RESOLUTION'}else if(d.windowTraded){eh.textContent='✓ ALREADY ENTERED THIS WINDOW'}else{eh.textContent='READY · SCANNING FOR UNDERDOG ≤ 0.20'};
