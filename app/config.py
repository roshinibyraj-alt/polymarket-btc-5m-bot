...
#          50 shares, only if that side is priced below 0.40 (same
#          entry logic as phase 1, different timing window).
...
# Only takers pay -- every buy/sell in this bot is treated as a taker
# fill (market orders / immediate fills), so the fee applies to both.
# Redemption at expiry is not a matched trade and is not fee'd.
# 0.07 is the current Crypto-category rate (see docs.polymarket.com/trading/fees
# -- verify this hasn't changed before relying on it for real capital).
TAKER_FEE_RATE = 0.07
