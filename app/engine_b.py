...
from .paper_broker import PaperBroker, compute_taker_fee
...
class EngineB:
    ...
    def _side_summary(self, side: Side, mark_price: Optional[float]) -> dict:
        ...
        fees_paid = sum(compute_taker_fee(f.shares, f.entry_price) for f in side_fills)
        avg_price = (cost / shares) if shares else None
        mark_value = (shares * mark_price) if (mark_price is not None and shares) else None
        unrealized_pnl = (mark_value - cost - fees_paid) if mark_value is not None else None
        return {
            "shares": shares,
            "avg_price": avg_price,
            "cost": cost,
